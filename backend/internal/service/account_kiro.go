...
// KiroIDToken 返回 id_token（仅 IdC 流程通常会颁发）。
func (a *Account) KiroIDToken() string {
	if !a.IsKiro() {
		return ""
	}
...
// KiroIsCapped 判断 Kiro 账号是否已用尽免费/付费配额（不在调度范围内）。
//
// 判定规则（参照 kiro-account-manager core/auto_switch.rs）：
//  1. 读取 Extra.kiro_usage_data.usageBreakdownList[0]
//  2. 当 currentUsage >= usageLimit 且 overageConfiguration.overageEnabled 为 false（或缺失）
//     视为 CAPPED。
//
// 任何字段缺失/类型不匹配均返回 false（保守不影响调度）。
func (a *Account) KiroIsCapped() bool {
